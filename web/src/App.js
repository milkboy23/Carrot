...
import { usePressedKeys, drawElement, adjustElementCoordinates, getElementAtPosition, resizedCoordinates, cursorForPosition, createElement, adjustmentRequired, getStrokeBounds, getRandomId, updateCursor, scaleCanvas, shouldDeleteElement, loadFont, } from "./utils";
...
import "./App.css";
const App = () => {
    const { tool, color, action, setAction, activeElement, setActiveElement, selectedElements, setSelectedElements, panOffset, setPanOffset, startPanMousePosition, setStartPanMousePosition, pendingEraseIds, setPendingEraseIds, dimensions, setDimensions, isInitialized, setIsInitialized, } = useAppState();
    const { elements, setElements, undo } = useHistory();
    const [isAppReady, setIsAppReady] = useState(false);
    const textAreaRef = useRef(null);
    const pressedKeys = usePressedKeys();
    const multiMoveStart = useRef(null);
    ...
    const drawAllElements = useCallback((canvas, context, width, height, withOffset = true) => {
        ...
        elements.forEach((element) => {
            if (action === Action.Writing && activeElement?.id === element.id)
                return;
            drawElement(roughCanvas, context, element, pendingEraseIds, selectedElements);
        });
    ...
    const redrawCanvas = useCallback(() => {
        if (!isAppReady)
            return; // Don't render until app is ready
    ...
    const moveElement = (element, deltaX, deltaY) => {
        ...
        if (result.x1 !== undefined)
            result.x1 = result.x1 + deltaX;
        if (result.y1 !== undefined)
            result.y1 = result.y1 + deltaY;
        if (result.x2 !== undefined)
            result.x2 = result.x2 + deltaX;
        if (result.y2 !== undefined)
            result.y2 = result.y2 + deltaY;
        ...
        if (result.strokes) {
            result.strokes = result.strokes.map((stroke) => stroke.map((p) => ({
                x: p.x + deltaX,
                y: p.y + deltaY,
            })));
        }
        ...
        if ([
            ...
            const recreated = createElement(result.id, result.x1, result.y1, result.x2, result.y2, result.type, result.color, result.src, result.initialCoordinates);
        ...
        if (result.type === TypesTools.Group && result.groupedElements) {
            result.groupedElements = result.groupedElements.map((child) => moveElement(child, deltaX, deltaY));
        }
        return result;
    };
    ...
    const updateElement = (id, x1, y1, x2, y2, type, options) => {
        ...
        switch (type) {
            ...
            case TypesTools.Circle:
                updated = createElement(id, x1, y1, x2, y2, type, elementsCopy[index].color, undefined, elementsCopy[index].initialCoordinates);
                elementsCopy[index] = updated;
                break;
            case TypesTools.Image:
                updated = createElement(id, x1, y1, x2, y2, type, undefined, elementsCopy[index].src);
                elementsCopy[index] = updated;
                break;
            ...
            case TypesTools.Arrow:
                updated = createElement(id, x1, y1, x2, y2, type, elementsCopy[index].color);
                elementsCopy[index] = updated;
                break;
            case TypesTools.Text: {
                ...
                updated = {
                    ...createElement(id, x1, y1, x1 + textWidth, y1 + textHeight, type, elementsCopy[index].color),
                    text: options?.text,
                };
                elementsCopy[index] = updated;
                break;
        ...
        if (updated && selectedElements.some((el) => el.id === id)) {
            const nextSelectedElements = selectedElements.map((el) => el.id === id ? updated : el);
    ...
    const onSaveCanvas = () => {
        ...
        elements.forEach((e) => {
            if (e.type === TypesTools.Pencil) {
                ...
                if (stokeBounds.minX < minX)
                    minX = stokeBounds.minX;
                if (stokeBounds.minY < minY)
                    minY = stokeBounds.minY;
                if (stokeBounds.maxX > maxX)
                    maxX = stokeBounds.maxX;
                if (stokeBounds.maxY > maxY)
                    maxY = stokeBounds.maxY;
            }
            else {
                if (e.x1 < minX)
                    minX = e.x1;
        ...
        const normalizeElement = (element) => {
            ...
            if (element.type === TypesTools.Group && element.groupedElements) {
            ...
            else if ([TypesTools.Pencil, TypesTools.Image, TypesTools.Text].includes(element.type)) {
                return {
                    ...element,
                    x1: element.x1 + deltaX,
                    y1: element.y1 + deltaY,
                    ...
                    strokes: element.strokes?.map((stroke) => stroke.map((point) => ({
                        x: point.x + deltaX,
                        y: point.y + deltaY,
                    }))),
                };
                ...
                return createElement(element.id, element.x1 + deltaX, element.y1 + deltaY, element.x2 + deltaX, element.y2 + deltaY, element.type, element.color, element.src, element.initialCoordinates);
            }
        };
        const elementsNormalized = elements.map(normalizeElement);
        const roughCanvas = rough.canvas(canvas);
        elementsNormalized.forEach((element) => {
            drawElement(roughCanvas, context, element, pendingEraseIds, [] // Pass empty array to avoid drawing selection boxes in saved image
            );
        });
        //*****/
        ...
        if (tool === TypesTools.Selection) {
            if (element) {
                if (pressedKeys.has("Shift")) {
                    const alreadySelected = selectedElements.some((el) => el.id === element.id);
                    if (alreadySelected) {
            ...
            const element = createElement(id, clientX, clientY - (tool === TypesTools.Text ? 28 / 2 : 0), clientX, clientY - (tool === TypesTools.Text ? 28 / 2 : 0), tool, color, undefined, {
                x1: clientX,
                y1: clientY,
            });
    ...
    const handleMouseMove = (event) => {
        ...
        if (action === Action.Drawing) {
            ...
            updateElement(id, initialCoordinates?.x1 || x1, initialCoordinates?.y1 || y1, clientX, clientY, tool);
        }
            ...
            if (selectedElements.length > 1) {
                if (multiMoveStart.current) {
                    ...
                    const elementsCopy = elements.map((el) => {
                        ...
                        if (el.type === TypesTools.Pencil) {
                            ...
                            const newStrokes = el.strokes?.map((stroke) => stroke.map((p) => ({
                                x: p.x + deltaX,
                                y: p.y + deltaY,
                            })));
                            return {
                        ...
                        if ([
                            ...
                            return createElement(el.id, el.x1 + deltaX, el.y1 + deltaY, el.x2 + deltaX, el.y2 + deltaY, el.type, el.color, el.src, el.initialCoordinates);
                        }
                        if (el.type === TypesTools.Group && el.groupedElements) {
                            return {
                                ...el,
                                groupedElements: el.groupedElements.map((child) => moveElement(child, deltaX, deltaY)),
                                x1: el.x1 + deltaX,
                                y1: el.y1 + deltaY,
                                x2: el.x2 + deltaX,
                                y2: el.y2 + deltaY,
                    ...
                    setElements(elementsCopy, true);
                    const nextSelectedElements = selectedElements.map((sel) => elementsCopy.find((el) => el.id === sel.id) || sel);
                    setSelectedElements(nextSelectedElements);
                }
            }
                ...
                const elementsCopy = [...elements];
                const elementId = elementsCopy.findIndex((e) => e.id === activeElement.id);
                elementsCopy[elementId] = {
                    ...elementsCopy[elementId],
                    points: newPoints,
                };
                setElements(elementsCopy, true);
                const nextSelectedElements = selectedElements.map((sel) => sel.id === activeElement.id ? elementsCopy[elementId] : sel);
                setSelectedElements(nextSelectedElements);
            }
            else if (activeElement?.type === TypesTools.Pencil &&
                activeElement.strokes) {
                ...
                const newStrokes = strokes?.map((stroke) => stroke.map((p) => ({
                    x: p.x + deltaX,
                    y: p.y + deltaY,
                })));
                const elementsCopy = [...elements];
                ...
                setElements(elementsCopy, true);
                const nextSelectedElements = selectedElements.map((sel) => sel.id === id ? elementsCopy[elementIndex] : sel);
                setSelectedElements(nextSelectedElements);
            }
            else if (activeElement?.type === TypesTools.Group) {
                // Group element - move all children
                const { id, x1, y1, x2, y2, offsetX, offsetY, groupedElements } = activeElement;
                const newX1 = clientX - (offsetX || 0);
                const newY1 = clientY - (offsetY || 0);
                const deltaX = newX1 - x1;
                const deltaY = newY1 - y1;
                const newGroupedElements = groupedElements?.map((child) => moveElement(child, deltaX, deltaY));
                const elementsCopy = [...elements];
                const elementIndex = elementsCopy.findIndex((e) => e.id === id);
                elementsCopy[elementIndex] = {
                    ...elementsCopy[elementIndex],
                ...
                const newX1 = clientX - (offsetX || 0);
                const newY1 = clientY - (offsetY || 0);
                const options = type === TypesTools.Text ? { text: activeElement?.text } : {};
            ...
            const { x1, y1, x2, y2 } = resizedCoordinates(clientX, clientY, position, {
                x1: coordinates.x1,
                y1: coordinates.y1,
                x2: coordinates.x2,
                y2: coordinates.y2,
    ...
    const handleMouseUp = (event) => {
        ...
        if (activeElement) {
            if (activeElement.type === TypesTools.Text &&
        ...
        if (action === Action.Writing)
            return;
        if (action === Action.Erasing) {
            setElements((prevState) => prevState.filter((e) => !pendingEraseIds.includes(e.id)));
            setPendingEraseIds([]);
            setAction(Action.None);
    ...
    if (!isAppReady) {
        return (<LoadingScreen width={dimensions.width} height={dimensions.height}/>);
    }
    return (<div style={{ width: dimensions.width, height: dimensions.height }}>
      {action === Action.Writing && (<textarea ref={textAreaRef} onBlur={handleBlur} onKeyDown={handleKeyDown} style={{
      ...
      <canvas id="canvas" width={dimensions.width} height={dimensions.height} onMouseDown={handleMouseDown} onMouseMove={handleMouseMove} onMouseUp={handleMouseUp} onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave} className="absolute z-[1]" onContextMenu={(e) => e.preventDefault()}/>
      <Toolbar onSaveCanvas={onSaveCanvas}/>
      <Minimap elements={elements} panOffset={panOffset} viewportWidth={dimensions.width} viewportHeight={dimensions.height} onNavigate={(x, y) => setPanOffset({ x, y })}/>
    </div>);
};
export default App;
