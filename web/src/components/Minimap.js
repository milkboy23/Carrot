...
const Minimap = ({ elements, panOffset, viewportWidth, viewportHeight, onNavigate, }) => {
                ...
                case TypesTools.Pencil: {
                    ...
                    if (element.strokes && element.strokes.length > 0) {
                        element.strokes.forEach((strokePoints) => {
                            ...
                            const stroke = getSvgPathFromStroke(getStroke(scaledPoints, {
    ...
    return (<canvas ref={canvasRef} className="minimap" width={MINIMAP_WIDTH} height={MINIMAP_HEIGHT} onClick={handleClick} style={{
            width: MINIMAP_WIDTH,
