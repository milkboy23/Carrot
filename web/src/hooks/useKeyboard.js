...
import { getRandomColor, getRandomId, updateCursor, getStrokeBounds, createElement, } from "../utils";
export const useKeyboard = (onSaveCanvas) => {
    const { undo, redo, elements, setElements } = useHistory();
    const { setTool, setColor, selectedElements, setSelectedElements, setCopiedElements, copiedElements, } = useAppState();
    ...
    useHotkeys("backspace", () => {
        if (selectedElements.length) {
            setElements(elements.filter((element) => !selectedElements.some((sel) => sel.id === element.id)));
            setSelectedElements([]);
        }
    });
    useHotkeys(["ctrl+c", "meta+c"], () => {
        setCopiedElements(elements.filter((element) => selectedElements.some((sel) => sel.id === element.id)));
    });
    ...
    useHotkeys(["ctrl+g", "meta+g"], (e) => {
        ...
        const toGroup = elements.filter((element) => selectedElements.some((sel) => sel.id === element.id));
        if (toGroup.length < 2)
            return;
        ...
        const remainingElements = elements.filter((element) => !selectedElements.some((sel) => sel.id === element.id));
    ...
    useHotkeys(["ctrl+shift+g", "meta+shift+g"], (e) => {
        ...
        const groupedElements = elements.filter((el) => selectedElements.some((sel) => sel.id === el.id) &&
            el.groupedElements &&
        ...
        groupedElements.forEach((grouped) => {
            ...
            grouped.groupedElements.forEach((original) => {
                if (original.type === TypesTools.Pencil) {
                    ...
                    origMinX = Math.min(origMinX, original.x1, original.x2 ?? original.x1);
                    origMinY = Math.min(origMinY, original.y1, original.y2 ?? original.y1);
                }
            });
...
function applyDeltaToElement(element, deltaX, deltaY) {
    ...
    if (result.type === TypesTools.Group && result.groupedElements) {
        result.groupedElements = result.groupedElements.map((child) => applyDeltaToElement(child, deltaX, deltaY));
    }
    return result;
}
