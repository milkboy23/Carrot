...
const getRoughOptions = (color) => ({
    stroke: color,
    strokeWidth: 1.8,
    roughness: 1.2,
});
export const createElement = (id, x1, y1, x2, y2, type, color, src, initialCoordinates) => {
    switch (type) {
        case TypesTools.Line: {
            const roughElement = generator.line(x1, y1, x2, y2, getRoughOptions(color));
            return { id, x1, y1, x2, y2, type, roughElement, color };
        }
        case TypesTools.Rectangle: {
            const roughElement = generator.rectangle(x1, y1, x2 - x1, y2 - y1, getRoughOptions(color));
            return { id, x1, y1, x2, y2, type, roughElement, color };
        }
        case TypesTools.Circle: {
            ...
            const roughElement = generator.circle(centerX, centerY, diameter, getRoughOptions(color));
            return {
                id,
                x1: realX1,
                y1: realY1,
        ...
        case TypesTools.Arrow: {
            ...
            const arrowHead1 = generator.line(x2, y2, x2 - 15 * Math.cos(Math.atan2(y2 - y1, x2 - x1) - Math.PI / 6), y2 - 15 * Math.sin(Math.atan2(y2 - y1, x2 - x1) - Math.PI / 6), getRoughOptions(color));
            const arrowHead2 = generator.line(x2, y2, x2 - 15 * Math.cos(Math.atan2(y2 - y1, x2 - x1) + Math.PI / 6), y2 - 10 * Math.sin(Math.atan2(y2 - y1, x2 - x1) + Math.PI / 6), getRoughOptions(color));
    ...
    switch (type) {
        ...
        case TypesTools.Pencil: {
            ...
            if (element.strokes && element.strokes.length > 0) {
                const betweenAnyPoint = element.strokes.some((stroke) => stroke.some((point, index) => {
        ...
        case TypesTools.Group: {
            ...
            if (element.groupedElements) {
                const isInsideAnyChild = element.groupedElements.some((child) => positionWithinElement(x, y, child) !== null);
...
const calculateAspectRatioAdjustment = (originalWidth, originalHeight, newWidth, newHeight) => {
...
const ensureMinSize = (position, coords) => {
...
export const resizedCoordinates = (clientX, clientY, position, coordinates, maintainAspectRatio = false) => {
    ...
    switch (position) {
        ...
        case "start": {
            if (maintainAspectRatio) {
                const { width, height } = calculateAspectRatioAdjustment(x2 - x1, y2 - y1, x2 - clientX, y2 - clientY);
        ...
        case "tr": {
            if (maintainAspectRatio) {
                const { width, height } = calculateAspectRatioAdjustment(x2 - x1, y2 - y1, clientX - x1, y2 - clientY);
        ...
        case "bl": {
            if (maintainAspectRatio) {
                const { width, height } = calculateAspectRatioAdjustment(x2 - x1, y2 - y1, x2 - clientX, clientY - y1);
        ...
        case "end": {
            if (maintainAspectRatio) {
                const { width, height } = calculateAspectRatioAdjustment(x2 - x1, y2 - y1, clientX - x1, clientY - y1);
...
const drawAnchors = (context, x1, y1, x2, y2) => {
    const padding = 3;
...
export const drawElement = (roughCanvas, context, element, pendingEraseIds, selectedElements) => {
            ...
            if (element.strokes && element.strokes.length > 0) {
                element.strokes.forEach((strokePoints) => {
                    if (strokePoints.length < 2)
                        return;
                    const stroke = getSvgPathFromStroke(getStroke(strokePoints, {
                        size: 4,
                        thinning: 0.5,
                        smoothing: 0.5,
                        streamline: 0.5,
        ...
        case TypesTools.Group:
            ...
            if (element.groupedElements) {
                element.groupedElements.forEach((child) => {
                    drawElement(roughCanvas, context, child, pendingEraseIds, selectedElements);
                });
            }
            break;
        default:
...
export const adjustmentRequired = (type) => [TypesTools.Line, TypesTools.Rectangle].includes(type);
...
export function getRandomColor() {
    ...
    while (true) {
        const randomColor = Color.rgb(Math.floor(Math.random() * 256), Math.floor(Math.random() * 256), Math.floor(Math.random() * 256));
...
export const updateCursor = (tool) => {
...
export const shouldDeleteElement = (element) => {
    const SIZE = 10;
...
export const loadFont = (fontFamily) => {
    return new Promise((resolve) => {
        ...
        // Load the font
        const font = new FontFace(fontFamily, `url(./assets/${fontFamily}.ttf) format('truetype')`);
        font
            .load()
            .then((loadedFont) => {
